...
import { FQ, FQ12, Bn128CurvePoint, Bn128CurvePointFQ12 } from './bn128_curve.js'
...
function cast_point_to_fq12(pt){
    if (pt === null)
        return null
    const [x, y] = pt.P
    return new Bn128CurvePointFQ12( 
...
function miller_loop(Q, P){
    ...
    const Q1 = new Bn128CurvePointFQ12 ( Q.x.pow(FQ.modulus), Q.y.pow(FQ.modulus) ) 
    const nQ2 = new Bn128CurvePointFQ12( Q1.x.pow(FQ.modulus), Q1.y.pow(FQ.modulus).neg() )
    f = f.mul( linefunc(R, Q1, P) )
    R = R.add(Q1)
    f = f.mul( linefunc(R, nQ2, P) )
    // R = add(R, nQ2) // This line is in many specifications but it technically does nothing
    return f.pow( (FQ.modulus ** 12n - 1n) / Bn128CurvePoint.order )
}
...
export function pairing(Q, P){
    Q.is_well_defined()
    P.is_well_defined()
    return miller_loop(Bn128CurvePointFQ12.twist(Q), cast_point_to_fq12(P))
}

function final_exponentiate(p){
    return p.pow( (FQ.modulus ** 12n - 1n)/Bn128CurvePoint.order ) 
}