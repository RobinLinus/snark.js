...
import { FQ, FQ12, BLSCurvePoint, BLSCurvePointFQ12 } from './bls_curve.js'


const ate_loop_count = 15132376222941642752n
const log_ate_loop_count = 61
...
function miller_loop(Q, P){
    ...
    const Q1 = new BLSCurvePointFQ12 ( Q.x.pow(FQ.modulus), Q.y.pow(FQ.modulus) ) 
    const nQ2 = new BLSCurvePointFQ12( Q1.x.pow(FQ.modulus), Q1.y.pow(FQ.modulus).neg() )
    f = f.mul( linefunc(R, Q1, P) )
    R = R.add(Q1)
    f = f.mul( linefunc(R, nQ2, P) )
    // R = add(R, nQ2) // This line is in many specifications but it technically does nothing
    return f.pow( (FQ.modulus ** 12n - 1n) / BLSCurvePoint.order )
}
...
export function pairing(Q, P){
    Q.is_well_defined()
    P.is_well_defined()
    return miller_loop(BLSCurvePointFQ12.twist(Q), cast_point_to_fq12(P))
}

function final_exponentiate(p){
    return p.pow( (FQ.modulus ** 12n - 1n)/BLSCurvePoint.order ) 
}
